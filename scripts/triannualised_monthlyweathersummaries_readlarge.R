...
triannual_yearmap <- bind_rows(
  ...
  dplyr::filter(WindowYear > min(climvar_cut$Year) + 1)
...
annual_sum <- climvar_cut %>%
  ...
  dplyr::inner_join(triannual_yearmap, by = c(YearFinishAug = "MeasumentYear")) %>%
...
# save data
saveRDS(annual_sum %>% dplyr::filter(WindowYear >= 1997), file = "./private/data/raw/triannualised_climvar_bggw_other_sites.rds")
