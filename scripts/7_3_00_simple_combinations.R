...
modelspecs <- sapply(names(modelspecs), function(x) {
  ...
  modspec$filename <- paste0("./tmpdata/7_3_00_clim_", x,".rds")
  if (grepl("_2lv$", x)) {modspec$nlv <- 2} else {modspec$nlv = 0}
  return(modspec)},
  USE.NAMES = TRUE,
  simplify = FALSE)
saveRDS(modelspecs, "./tmpdata/7_3_00_modelspecs.rds")
...
saveRDS(lpds, file = "./tmpdata/7_3_00_lpds.rds")
...
saveRDS(waics, file = "./tmpdata/7_3_00_waics.rds")
parallel::stopCluster(cl)

loo_warnings <- warnings()
saveRDS(loo_warnings, file = "./tmpdata/WAICS/7_3_00_warnings.rds")
