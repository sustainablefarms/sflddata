...
modelspecs <- list(
  latlon             = list(OccFmla = "~ 1 + longitude * latitude",
                            ObsFmla = "~ 1"),
  latlon_year        = list(OccFmla = "~ 1 + longitude * latitude + SurveyYear",
                            ObsFmla = "~ 1"),
  year               = list(OccFmla = "~ 1 + SurveyYear",
                            ObsFmla = "~ 1"),
  AnnTemp_AnnPrec    = list(OccFmla = "~ 1 + AnnMeanTemp + AnnPrec", #meanclimhyp
                            ObsFmla = "~ 1"),
  MaxT_PrecWarmQ     = list(OccFmla = "~ 1 + MaxTWarmMonth + PrecWarmQ", #aridhyp
                            ObsFmla = "~ 1"),
  MinT_PrecColdQ     = list(OccFmla = "~ 1 + MinTColdMonth + PrecColdQ", #coldlimithyp
                            ObsFmla = "~ 1"),
...
modelspecs <- sapply(names(modelspecs), function(x) {
  ...
  modspec$filename <- paste0("./tmpdata/7_2_10_clim_", x,".rds")
  if (grepl("_2lv$", x)) {modspec$nlv <- 2} else {modspec$nlv = 0}
  return(modspec)},
  USE.NAMES = TRUE,
  simplify = FALSE)
saveRDS(modelspecs, "./tmpdata/7_2_10_modelspecs.rds")
...
saveRDS(lpds, file = "./tmpdata/7_2_10_lpds.rds")
...
saveRDS(waics, file = "./tmpdata/7_2_10_waics.rds")
parallel::stopCluster(cl)

loo_warnings <- warnings()
saveRDS(loo_warnings, file = "./tmpdata/WAICS/7_2_10_warnings.rds")
