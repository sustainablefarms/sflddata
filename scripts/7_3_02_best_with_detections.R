...
# using "~ 1 + MeanTime + MeanTemp + MeanWind + MeanClouds"
# occ: all 4 combinations + 3 individually + best in each class
...
modelspecs <- list(
  ...
  woody500m = 
    list(OccFmla = "~ 1 + woody500m", 
         ObsFmla = "~ 1 + MeanTime + MeanTemp + MeanWind + MeanClouds"),
  msnm = 
    list(OccFmla = "~ 1 + ms * NMdetected", 
         ObsFmla = "~ 1 + MeanTime + MeanTemp + MeanWind + MeanClouds"),
  
  someclimate_year_msnm = 
    list(OccFmla = "~ 1 + AnnMeanTemp + AnnPrec + AnnTempRange + PrecSeasonality +
        SurveyYear + PrecWarmQ + ms * NMdetected", 
         ObsFmla = "~ 1 + MeanTime + MeanTemp + MeanWind + MeanClouds"),
  someclimate_year_woody500m = 
    list(OccFmla = "~ 1 + AnnMeanTemp + AnnPrec + AnnTempRange + PrecSeasonality +
        SurveyYear + PrecWarmQ + woody500m", 
         ObsFmla = "~ 1 + MeanTime + MeanTemp + MeanWind + MeanClouds"),
  someclimate_year_woody500m_msnm = 
    list(OccFmla = "~ 1 + AnnMeanTemp + AnnPrec + AnnTempRange + PrecSeasonality +
        SurveyYear + PrecWarmQ + woody500m + ms * NMdetected", 
         ObsFmla = "~ 1 + MeanTime + MeanTemp + MeanWind + MeanClouds"),
  woody500m_msnm = 
    list(OccFmla = "~ 1 + woody500m + ms * NMdetected", 
         ObsFmla = "~ 1 + MeanTime + MeanTemp + MeanWind + MeanClouds"),
...
names(modelspecs) <- paste0(names(modelspecs), "_det1stO")
...
modelspecs <- sapply(names(modelspecs), function(x) {
  ...
  USE.NAMES = TRUE,
  simplify = FALSE)
saveRDS(modelspecs, "./tmpdata/7_3_02_modelspecs.rds")

devtools::load_all()
indata <- readRDS("./private/data/clean/7_2_10_input_data.rds")
modelspecs <- readRDS("./tmpdata/7_3_02_modelspecs.rds")
runfun <- function(x) {
...
saveRDS(lpds, file = "./tmpdata/7_3_02_lpds.rds")
...
saveRDS(waics, file = "./tmpdata/7_3_02_waics.rds")
parallel::stopCluster(cl)

loo_warnings <- warnings()
saveRDS(loo_warnings, file = "./tmpdata/WAICS/7_3_02_warnings.rds")
