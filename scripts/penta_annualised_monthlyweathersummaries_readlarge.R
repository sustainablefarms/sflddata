...
# first create a table with two years for each year.
Years = min(climvar_cut$Year) : max(climvar_cut$Year)
pentaannual_yearmap <- bind_rows(
  data.frame(WindowYear = Years, MeasumentYear = Years),
  data.frame(WindowYear = Years, MeasumentYear = Years - 1),
  data.frame(WindowYear = Years, MeasumentYear = Years - 2),
  data.frame(WindowYear = Years, MeasumentYear = Years - 3),
  data.frame(WindowYear = Years, MeasumentYear = Years - 4)
  ) %>%
  dplyr::arrange(WindowYear, MeasumentYear) %>%
  dplyr::filter(WindowYear > min(climvar_cut$Year) + 4)
...
annual_sum <- climvar_cut %>%
  ...
  dplyr::inner_join(pentaannual_yearmap, by = c(YearFinishAug = "MeasumentYear")) %>%
  dplyr::arrange(WindowYear) %>% #distinct(YearFinishAug, WindowYear) %>% View()
  dplyr::group_by(StudyCode, SiteCode, WindowYear) %>%
...
# save data
saveRDS(annual_sum %>% dplyr::filter(WindowYear >= 1997), file = "./private/data/raw/pentaannualised_climvar_bggw_other_sites.rds")
