...
modelspecs <- list(
  someclimate_year_woody500m_msnm_time = 
    list(OccFmla = "~ 1 + AnnMeanTemp + AnnPrec + AnnTempRange + PrecSeasonality +
        SurveyYear + PrecWarmQ + woody500m + ms * NMdetected", 
         ObsFmla = "~ 1 + MeanTime"),
  someclimate_year_woody500m_msnm_time2 = 
    list(OccFmla = "~ 1 + AnnMeanTemp + AnnPrec + AnnTempRange + PrecSeasonality +
        SurveyYear + PrecWarmQ + woody500m + ms * NMdetected", 
         ObsFmla = "~ 1 + MeanTime + I(MeanTime^2)"),
  someclimate_year_woody500m_msnm_time_temp = 
    list(OccFmla = "~ 1 + AnnMeanTemp + AnnPrec + AnnTempRange + PrecSeasonality +
        SurveyYear + PrecWarmQ + woody500m + ms * NMdetected", 
         ObsFmla = "~ 1 + MeanTime + MeanTemp"),
  someclimate_year_woody500m_msnm_timetemp = 
    list(OccFmla = "~ 1 + AnnMeanTemp + AnnPrec + AnnTempRange + PrecSeasonality +
        SurveyYear + PrecWarmQ + woody500m + ms * NMdetected", 
         ObsFmla = "~ 1 + MeanTime * MeanTemp"),
...
modelspecs <- sapply(names(modelspecs), function(x) {
  ...
  modspec$filename <- paste0("./tmpdata/7_3_01_clim_", x,".rds")
  if (grepl("_2lv$", x)) {modspec$nlv <- 2} else {modspec$nlv = 0}
  return(modspec)},
  USE.NAMES = TRUE,
  simplify = FALSE)
saveRDS(modelspecs, "./tmpdata/7_3_01_modelspecs.rds")

devtools::load_all()
indata <- readRDS("./private/data/clean/7_2_10_input_data.rds")
modelspecs <- readRDS("./tmpdata/7_3_01_modelspecs.rds")
runfun <- function(x) {
...
saveRDS(lpds, file = "./tmpdata/7_3_01_lpds.rds")
...
saveRDS(waics, file = "./tmpdata/7_3_01_waics.rds")
parallel::stopCluster(cl)

loo_warnings <- warnings()
saveRDS(loo_warnings, file = "./tmpdata/WAICS/7_3_01_warnings.rds")
