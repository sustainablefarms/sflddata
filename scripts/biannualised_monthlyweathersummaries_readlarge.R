...
biannual_yearmap <- bind_rows(data.frame(WindowYear = Years, MeasumentYear = Years),
  data.frame(WindowYear = Years, MeasumentYear = Years - 1)) %>%
  dplyr::arrange(WindowYear, MeasumentYear) %>%
  dplyr::filter(WindowYear != min(climvar_cut$Year))
...
annual_sum <- climvar_cut %>%
  ...
  dplyr::inner_join(biannual_yearmap, by = c(YearFinishAug = "MeasumentYear")) %>%
...
# save data
saveRDS(annual_sum %>% dplyr::filter(WindowYear >= 1997), file = "./private/data/raw/biannualised_climvar_bggw_other_sites.rds")
