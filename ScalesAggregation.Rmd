...
$$\sum_{k = 1}^K p_k = \sum_{k = 1}^K E(D_k) = E\left(\sum_{k = 1}^K D_k\right) =r$$
is the expected number of species detected, $r$.


If there is dependence between species, and this dependence follows specific structures then distributional information for the number of species detected $\sum_{k = 1}^K D_k$ may be possible using simulations.
...
Inspection of their code suggests their simulation used samples from the mcmc chain for covariate coefficients.
...
*In theory such a method could easily be modified to simulate detected species at multiple locations in a farm and aggregate to species richness across a farm. However, in practise such simulations could be too costly to do at all desired locations: for each mcmc sample it would involve simulating 1000 collections of detected species for every location (one for each latent variable value).*
