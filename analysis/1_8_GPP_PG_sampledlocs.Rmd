---
title: "GPP vs PG fit at Random Locations"
...
```
```{r loadwoody}
woody_b <- raster("./tmpdata/woody_2000.grd")
```

Ideally would take the sum of each pixel of woody cover within a pixel of the GPP data. 
however it seems that software for it is a step too clunky. spdf grids might do it though.
Below uses aggregation and projection functions. It has worked ok :(.

```{r atemporaryGPPmap_for_CRS}
...
```{r rastersmatch}
all.equal(gpp_b, pg_b, value = FALSE, res = TRUE)
all.equal(gpp_b, woody_b5, value = FALSE, res = TRUE)
```
...
locs$woody_b5 <- extract(woody_b5, locs)
locs <- locs %>% 
  filter(!is.na(woody_b5)) %>%
  filter(woody_b5 <= 100) %>%  #fairly sure 100+ values represent no data values
  filter(woody_b5 >= 0)

plot(woody_b5, main = "Training Locations")
...
par(mfrow = c(1, 2))
hist(woody_b5, breaks = c((0:10)*10, 170), col = "grey")
hist(locs$woody_b5, breaks = (0:10)*10, border = "red", lty = "dashed", freq = FALSE, add = TRUE)
qqplot(locs$woody_b5, matrix(woody_b5))
abline(a = 0, 1)
```
...
woody_locs <- st_as_sf(extract(woody_b5, locs, sp = TRUE))
pg_locs <- st_as_sf(extract(pg_b, locs, sp = TRUE))
rm(woody_b5, gpp_b, pg_b_projected)
```
...
datanice <- inner_join(datanice, woody %>% dplyr::select(pointid, woody), by = c("pointid"))
```
...
library(fable)
m12_sp <- ARIMA(as.formula(paste("box_cox(gpp / gpp.ydaymed, lambda = 0.1414141) ~", paste(mod8_vars, collapse = "+"), "+ pdq()")))
