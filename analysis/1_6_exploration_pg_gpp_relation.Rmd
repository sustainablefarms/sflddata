...
meanresid %>%
  ...
  coord_fixed() +
  ggtitle("Average Residual of m7 for each site")
...
train %>% filtertrain() %>% tibble::rowid_to_column() %>%
  ...
  ggplot() +
  facet_wrap(~site) +
  geom_line(aes(x = times, y = .resid)) +
  ggtitle("m7 innovation residuals by site")
```

The ARCH1 farm is a massive outlier in average residual. __Why?__ It appears common when plotted.
...
train %>% filtertrain() %>% tibble::rowid_to_column() %>%
  right_join(residuals(m7), by = "rowid") %>%
  ggplot() +
  facet_wrap(~year) +
  geom_hline(yintercept = 0) +
  geom_point(aes(x = yday, y = .resid))
