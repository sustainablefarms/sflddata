...
m6 %>%
  ...
  filter(farm %in% c("CAIN", "GEDD", "MATH", "SMAR")) %>% 
...
residuals(m6[[1, 1]], type = "regression") %>%
  ...
  update_tsibble(key = "site", index = "times") %>%
  as_tibble() %>%
  select(rowid, times, farm, `gpp/gpp.ydaymed`,
                 `gpp/gpp.ydaymed.pred`,
                 .resid.regression,
                 .resid.innovation
                 ) %>%
  filter(farm %in% c("CAIN")) %>% 
  mutate(resp.obs.m.pred = `gpp/gpp.ydaymed` - `gpp/gpp.ydaymed.pred`) %>%
  pivot_longer(c(`gpp/gpp.ydaymed`,
                 `gpp/gpp.ydaymed.pred`,
                 .resid.regression,
                 .resid.innovation)) %>%
  ggplot() +
  ...
  geom_line(aes(x = times, y = resp.obs.m.pred), col = "black", lty = "dotted") +
  geom_line(aes(x = times, y = value, col = name, lty = name)) 
...
residuals(m6[[1, 1]], type = "regression") %>%
  ...
  filter(farm %in% c("CAIN")) %>% 
  mutate(pred.p.lin.resid = lin.pred + lin.resid) %>%
  pivot_longer(c(
      lin.pred,
         lin.obs,
         lin.resid,
  ...
  #facet_wrap(~farm, scales = "free_y") +
  facet_grid(rows = vars(name)) +
  geom_line(aes(x = times, y = pred.p.lin.resid), col = "black", lty = "dotted") +
  geom_line(aes(x = times, y = value, col = name, lty = name)) 
...
There is definitely still structure in the residuals: clear correlation between residuals on each day. `yday` was not a predictor in the model.