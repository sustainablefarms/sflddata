...
# half the excluded birds feed in agricultural landscapes, yet 97% of inconsistent birds use agricultural landscapes to feed
...
```
This looks like it might be because alot of birds that sometimes eat vertebrates are being excluded by mistake. What we are really after are raptors and obligate waterbirds, so a taxonomic distinction might be more useful here.
...
birds_to_exclude <- as_tibble(traits) %>%
  dplyr::filter(X11_Order_2 %in% excluded_orders)
