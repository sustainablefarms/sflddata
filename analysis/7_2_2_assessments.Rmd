...
names(lppds)[10] <- "clouds_time_temp_wind"
lppds_df <- do.call(cbind, lapply(lppds, function(x) x$lpds))
lppds <- sort(vapply(lppds, function(x) mean(x[["lpds"]]), FUN.VALUE = 3.3))
print(lppds)
cat("max is:")
...
mlpds_df <- tibble(Model = names(lppds), Estimate = lppds, SE = NA, type = "Holdout")

df <- bind_rows(waics_average_elpd_per_point, loo_average_elpd_per_point, lppds_df)

# 
# df <- rbind(waics_average_elpd_per_point, loo_average_elpd_per_point)
# df <- rbind(df[, names(lppds)], meanholdoutlpd = lppds)

df %>%
  ggplot() +
  geom_errorbar(aes(x = Model, ymax = Estimate + SE, ymin = Estimate - SE, col = type)) +
  geom_point(aes(x = Model, y = Estimate, col = type)) +
  coord_flip()
...
df <- lppds_df %>% as_tibble() %>% mutate(InSample = FALSE)

df <- bind_rows(insamplelpds_df, df)

df %>%
  as_tibble() %>%
  tidyr::pivot_longer(-InSample, names_to = "model", values_to = "site_elpd") %>%
  ggplot() +
  geom_violin(aes(x = site_elpd, y = model, col = InSample))
```
...
lapply(v.b_l, function(x) colMeans(abs(x)))
```

It is very hard to compare covariate loadings between models, as the covariates are different.
However the covariates loadings are all nearly close to zero!!
This explains why the models are producing the same stuff. I feel a bit foolish!

MeanTime has the largest average loading.
