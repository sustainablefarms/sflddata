...
## Holdout lppd
```{r holdoutlppd}
lppds <- readRDS("~/Documents/professional/computerprogramming/Rlangauge/linking-data/tmpdata/7_2_2_lppds.rds")
lppds <- vapply(lppds, function(x) x[["lppd"]], FUN.VALUE = 3.3)
print(lppds)
cat("max is:")
lppds[which.max(lppds)]
```

Of the fitted objects that appeared to have good MCMC chains, the best lppd is the windtemp_time model.
