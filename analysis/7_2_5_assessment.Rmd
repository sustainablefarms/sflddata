...
```{r residocc_plots_manymodels, fig.width = 8}
...
The os_msnm_gc_2ndO model has the lowest residuals. However the ms:NMdetected residuals is messy and could suggest overfitting.
Thus it might be worth trying "~1 + os + I(os^2) + I(ms^2) + ms:NMdetected + gc + I(gc^2)".
Other interactions would also be interesting but would be good to test using residual plots first.
