...
residuals <- ds_detection_residuals.fit(fit, type = "median")
names(residuals)[-1] <- names(detection_data)[names(detection_data) %in% detection_data_specieslist]
covar <- detection_data[ , c("SiteID", "MeanWind"), drop = FALSE] %>%
  rename("ModelSite" = SiteID)

plot_residuals.residual(residuals, covar,
                 plotfunction = facet_covariate) + 
  scale_y_continuous(name = "Detection Residuals") +
...
pltobj <- plot_residuals.residual(residuals, covar,
  ...
  scale_y_continuous(name = "Detection Residuals")
pltobj
pltobj + coord_cartesian(ylim = c(-1, +1)) +
  geom_smooth(aes(x = CovariateValue, y = Residual), method = "lm", level = 0, formula = y ~ 1,
              col = "red", lty = "dashed")
```

The detection residuals are non-zero (strange for an included covariate), and the impact of high winds is similar to that of low winds.
Treaing each species independently:
the 95% region of the residuals smooth covers a constant for most species (Weebill is an exception).
This suggests that MeanWind has been suitably included in the model (no transformations or separations between wind speeds required).
I don't know why the residuals are non-zero for each species though. This is troubling and could be a mistake in my code, or assumptions.


```{r MeanWind_Residual}
covar <- detection_data[ , c("SiteID", "MeanTime"), drop = FALSE] %>%
  rename("ModelSite" = SiteID)
