...
```{r gedd2_multpeaks2, echo = FALSE, width = 14}
colrs <- viridis(length(unique(pggpp$site))/4)
names(colrs) <- unique(pggpp$site)[seq(1, 176, by = 4)]
colrs <- unlist(colrs)
colrs["GEDD1"] <- "red"
colrs["MATH1"] <- "orange"
pggpp %>%
  filter(site %in% names(colrs)) %>%
  filter_index("2009" ~ "2013") %>%
  pivot_longer(c(pg, gpp), names_to = "valname") %>%
  as_tsibble(key = c(site, valname), index = times) %>%
  mutate(value = zoo::na.approx(value, rule = 1, na.rm = FALSE)) %>%
  ...
  ggtitle("Observed GPP and PG of all farms")
```

It looks like GEDD1 (red) has unusally high GPP whilst having what appears to be slightly high rainfall.
