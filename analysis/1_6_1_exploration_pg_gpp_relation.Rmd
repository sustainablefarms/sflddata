...
## Find a better lambda for Box-Cox
```{r boxcoxlambda}
...
No new model appears to be able to reduce the autocorrelation.
...
acfs <- acf(residuals(m9))
m9
```

The autocorrelations of this model's innovations are much better than `m6`, even when the order_constraint is very high (e.g. m8 model fitting above). The annual correlation (lag = 16) is quite obvious.
