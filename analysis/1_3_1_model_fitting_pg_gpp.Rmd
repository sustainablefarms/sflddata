...
\[
\begin{aligned}
&& Y_i(t) = \theta_{10} + \big(\theta_{11} Y_i(t - 1yr) +  \theta_{12}Y_i(t - 2yr) +  \theta_{13}Y_i(t - 3yr) +  \theta_{14}Y_i(t - 4yr) + \theta_{15}Y_i(t - 5yr) \big) +\\
&& \big(\theta_{20} X_i(t) +  \theta_{22}X_i(t - 1w) + ... + \theta_{29}X_i(t - 9w) +
\epsilon_i(t)\\
\end{aligned}
\]
where 
...
+ $X_i$ is the sum of rain in the last 7 days
+ $t$ is the date
...
est_resid_training %>%
  ...
  ggtitle("Density of residual occurence through each GPP value of the prior year")
```
...
These non-normally distributed residuals suggest that the model form is not appropriate.
...
It is clear the residuals are autodependent which is against model assumptions (pdq set to 0, 0, 0).
...
I am curious to look at these years in more detail to determin if the differences between forecast GPP and observed GPP are correlated in some way to precipitation (or anything else) - this could suggest an element of the model that is missing.
...
1. An additive model is not sufficient - low rain needs to be able to have a negative effect on GPP.
2. 9 weeks (2 months) is too short lag for precipitation - better to use 3, 6 or even 8 months.
...
The year 2014 has a spike in GPP in autumn. This could be due to slightly high rainfall occuring a month earlier 2014.
...
However, the forecasts for test data are plotted below for curiousity.
