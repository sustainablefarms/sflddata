...
Xtestproto <- model.frame(~ pg_1to5m * I(1 / pg_1to5m.ydaymed) * pg_24d,
             data = test %>% filtertrain4() %>% filter(site == "WILS1"))
...
Ytest <- test %>%
  filtertrain4() %>% filter(site == "WILS1") %>%
  mutate(actual = box_cox(gpp / gpp.ydaymed, lambda = 0.1414141)) %>%
  as_tibble() %>%
  select(times, actual)
fcst_test <- ugarchforecast(garch_m1,
...
```{r forecast_GEDD1}
Xtestproto <- model.frame(~ pg_1to5m * I(1 / pg_1to5m.ydaymed) * pg_24d,
             data = test %>% filtertrain4() %>% filter(site == "GEDD1"))
Xtestproto <- scale(Xtestproto,
                    center = attr(Xproto, "scaled:center"),
                    scale = attr(Xproto, "scaled:scale"))
Xtest <- model.matrix(~ 
             ...
             data = data.frame(Xtestproto))[, -1]
Ytest <- test %>%
  filtertrain4() %>% filter(site == "GEDD1") %>%
  mutate(actual = box_cox(gpp / gpp.ydaymed, lambda = 0.1414141)) %>%
  as_tibble() %>%
  select(times, actual)
fcst_test <- ugarchforecast(garch_m1,
...
```{r forecast_LIND1}
Xtestproto <- model.frame(~ pg_1to5m * I(1 / pg_1to5m.ydaymed) * pg_24d,
             data = test %>% filtertrain4() %>% filter(site == "LIND1"))
Xtestproto <- scale(Xtestproto,
                    center = attr(Xproto, "scaled:center"),
                    scale = attr(Xproto, "scaled:scale"))
Xtest <- model.matrix(~ 
             ...
             data = data.frame(Xtestproto))[, -1]
Ytest <- test %>%
  filtertrain4() %>% filter(site == "LIND1") %>%
  mutate(actual = box_cox(gpp / gpp.ydaymed, lambda = 0.1414141)) %>%
  as_tibble() %>%
  select(times, actual)
fcst_test <- ugarchforecast(garch_m1,
...
Weird that the prediction for GEDD1, WILS1, and LIND1 are nearly identical up to a constant-looking offset, the covariates for each site would not be that similar.
This suggests that the model is ignoring the covariates.