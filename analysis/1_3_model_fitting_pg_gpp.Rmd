...
\[
\begin{aligned}
H1: && Y_i(t) = s_i(t_{yr}) + g\big(r_i(t), r_i(t-1), r_i(t-2), r_i(t-3), ...\big) + \epsilon_i(t)\\
H2: && Y_i(t) = s_i(t_{yr}) \times g\big(r_i(t), r_i(t-1), r_i(t-2), r_i(t-3), ...\big) \times \epsilon_i(t)
\end{aligned}
\]
where 
+ $Y_i(t)$ is the GPP at location $i$ on date $t$,
+ $t_{yr}$ is the date modulo year,
+ $s_i(t_{yr})$ is the amount of GPP due to factors that are constant across years (and could, for example be a function of past GPP),
...
+ spatial interactions for rainfall may be missed

## Model Fitting for General H2
Models where this a component that is uninteresting can sometimes be fitted using partial likelihood.
The next section presents computes the likelihood functions, but does not go further.
For this level of analysis it will be more appropriate to try existing well established methods, with programs available.
That is the topic of the subsequent section.

### Likelihood of the General Models
...
The likelihood of $y_i^j(t_j)$ assuming $H2$ is:
\[
\begin{align}
...
Such models might be estimated through:
...
## Fitting H1:
In the pursuit of fitting a quick and easy H1 model, I look to standard time series methods ARIMA.

It seems like `stats::arima` will fit ARIMA model including regressors! This could be applied to a single site quickly, and with more work could likely be tricked into applying to all sites. 
...
fitbell3 <- bell1_train %>%
  ungroup() %>%
  # model(withpg_bin7 = ARIMA(modelfmla3), withpg_bin7_hist5 = ARIMA(modelfmla4))
...
fit_fmla4_concatsites <- pggpp_train %>%
  model(ARIMA(modelfmla4))

saveRDS(fit_fmla4_concatsites, file = "fit_fmla4_concatsites.rds")

# plot forecasts for test data
fit_fmla4_concatsites %>%
...
modelfmla5 <- reformulate(c(paste0("lag(log(lininterp_gpp + 0.001), n = ", (1:5)*365.25, ")"),
                           "pdq(0, 0, 0)",
                           "PDQ(0, 0, 0)",
                           paste0("lag(slide_dbl(pg, sum, .size = 7, .align = 'r'), n = ", 7 * (0:9), ")")),
                           response = "log(gpp + 0.001)")
fit_fmla5_concatsites <- pggpp_train %>%
  model(ARIMA(modelfmla5))

saveRDS(fit_fmla5_concatsites, file = "fit_fmla5_concatsites.rds")

# plot forecasts for test data
fit_fmla5_concatsites %>%
  forecast(new_data = pggpp %>% 
             filter(site == "BELL1") %>% 
             update_tsibble(key = NULL, index = "times")
           ) %>%
