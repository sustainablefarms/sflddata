...
onlyseasonal <- arima(bell1_train_ts, xreg = fourier(bell1_train_ts, K = 10))
onlyseasonal
pred <- predict(onlyseasonal, n.ahead = 700,
        newxreg = fourier(bell1_train_ts, K = 10, h = 700),
        )
...
```{r fitlininterpGPP}
#create lagged PG columns
bell1 <- pggpp %>%
  filter(site == "BELL1")
pglags <- lapply(1:60, function(x) dplyr::lag(bell1$pg, x))
names(pglags) <- paste0("pglag", 1:60)
pglags <- as_tibble(as.matrix(data.frame(pglags))) %>%
  dplyr::mutate(times = bell1$times) %>%
  as_tsibble(index = times)

bell1 <- bell1 %>%
 inner_join(pglags, by = "times")
...
modelfmla <- reformulate(c("lag(lininterp_gpp, n = 365.25)",
                           "lag(lininterp_gpp, n = 2*365.25)",
                           "trend()",
                           "pdq(0, 0, 0)",
                           "PDQ(0, 0, 0)",
                           paste0("pglag", 1:5)), response = "gpp")
fit <- bell1_train %>%
  ungroup() %>%
...
fit %>%
  ...
  ggplot() +
  geom_ribbon(aes(x = times, ymin = interval$.lower, ymax = interval$.upper)) +
...
out2 <- bell1_train %>%
  ungroup() %>%
  model(ARIMA(lininterp_gpp ~ pglagsmat[, 1:2] + 1 + trend() + PDQ(0:3, 0:1, 0:2, period = "1 year"))) #trend() avoids model fits from erroring, but not sure why
report(out)
out %>%
  augment() %>%
  filter_index(. ~ "2004") %>%
  ggplot() +
  geom_point(aes(x = times, y = .resid))

inputdata <- cbind(bell1, laggedpg = lapply(1:60, function(x) dplyr::lag(bell1$pg, x)))
