...
bell1_train_ts <- as.ts(bell1_train, value = lininterp_gpp, frequency = 365.25)
# specify 365.25 time points per unit of time. So 1 unit of time = 1 year

#use fourier() to avoid the maximum lag = 350 warning
onlyseasonal <- arima(bell1_train_ts, xreg = fourier(bell1_train_ts, K = 5))
pred <- predict(onlyseasonal, n.ahead = 365,
        newxreg = fourier(bell1_train_ts, K = 5, h = 365),
        )
...
out <- bell1_train %>%
  ungroup() %>%
  model(ARIMA(lininterp_gpp ~ lag(pg, n = 1) + lag(pg, n = 2) + 1 + trend() + PDQ(2:3, 0:1, 0:2, period = "1 year"))) #trend() avoids model fits from erroring, but not sure why

pglags <- lapply(1:60, function(x) dplyr::lag(bell1_train$pg, x))
names(pglags) <- 1:60
pglagsmat <- as.matrix(data.frame(pglags))
