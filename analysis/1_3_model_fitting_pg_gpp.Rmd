...
modelfmla <- reformulate(c(paste0("lag(lininterp_gpp, n = ", (1:2)*365.25, ")"),
                           "trend()",
                           "pdq(0, 0, 0)",
                           "PDQ(0, 0, 0)",
                           paste0("lag(pg, n = ", 1:5, ")")),
...
# plot forecasts for test data
fit %>%
  forecast(new_data = pggpp_test %>% 
             select(-gpp) %>%  #just for sanity's sake
             filter(site == "ARCH1") %>% 
  ...
  geom_ribbon(aes(x = times, ymin = interval$.lower, ymax = interval$.upper), fill = "lightgrey") +
  geom_line(aes(x = times, y = gpp), col = "black") + 
  geom_point(aes(x = times, y = gpp), data = pggpp_test %>% filter(site == "ARCH1"), col = "blue")
```


+ Still to do: look up and determine whether fitting an ETS or 'exponential smoothing' model would make sense: https://otexts.com/fpp3/expsmooth.html