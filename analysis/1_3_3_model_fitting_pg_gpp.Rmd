...
(this is documented in 1_3_1_model_fitting_pg_gpp.Rmd and 1_3_2_model_fitting_pg_gpp.Rmd)
...
4. These questions could be modelled coarsely using interaction terms, but it would result in a lot of interaction terms - reducing them using physical thinking would speed up model fitting. A spline GAM-style thing might also work well but only avoids the interaction term if a cumulative rainfall is used.
...
1. correlation analysis, a variety of transforms and derivative precipitation information, would be useful for choosing predictors
...
+ Possible packages and models to consider (from Martin): 
  + perhaps even a GAM (spline) model. Fit gams with packages ‘gam’ or ‘mgcv’,
    + GAMs are linear models where splines (or other smooth basis-generated functions) are used to mediate the effect of a covariate.
    + examine pattern of the residuals with respect to covariate using gam.check in mgcv package
    + package mgcv allows gams with autocorrelated errors, even organised within groups (see examples of `mgcv::gamm`)
  + If you’re interested in mixed models, package ‘lme4’ is a good choice. 
    + mixed models are good when there are groups within the data, and that there is systematic effect of these groups. For example when the data is GPP and precipitation measurements from multiple sites. Each site is a group, and I would expect the GPP and precipitation to be dependent on the site.
    + autocorrelation in residuals possible: see appendix A of the paper on `lme4`.
  + `nlme` might be able to fit models with mixed-effects with autocorrelation in the residuals
  + Fit mixed model gams using the 'gam' package.